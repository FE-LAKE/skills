...
将 Figma 设计稿转换为生产级代码，并完成后续视觉评审与自动优化。Agent 会读取 Figma、生成代码、运行 lint/typecheck、截图对比实现与设计稿，并按 review brief 自动修复视觉差异。
...
6. 生成 review brief，并在宿主支持时优先交给 subagent 或等价隔离执行能力运行 review phase。
7. review phase 获取 Figma 参照图与实现页截图，自动修复视觉差异并复查。
...
完成代码生成与 lint/typecheck 后，先生成 review brief，再执行 [references/review-policy.md](references/review-policy.md)。宿主支持 subagent 或等价隔离执行能力时，优先把 brief 交给隔离执行；否则在当前会话中按同一份 brief 执行独立评审步骤。

review brief 必须包含：

- Figma URL 与目标 node
- 已生成或修改的文件列表
- dev server 启动命令与 dev URL（若已知）
- 视口、技术栈、重要 assumptions、lint/typecheck 结果
...
- review phase 优先使用 subagent 或等价隔离执行能力；不支持时按 review brief 在当前会话中独立执行。
